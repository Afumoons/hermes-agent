...
def test_sticky_weighted_strategy_keeps_current_healthy_credential(tmp_path, monkeypatch):
    """sticky_weighted should keep using the current healthy credential inside one pool instance."""
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    monkeypatch.setattr(
        "agent.credential_pool.get_pool_strategy",
        lambda _provider: "sticky_weighted",
    )
    monkeypatch.setattr(
        "agent.credential_pool._seed_from_singletons",
        lambda provider, entries: (False, set()),
    ...
    assert first is not None
    assert second is not None
    assert first.id == second.id == "secondary"
    assert second.request_count == first.request_count + 1


def test_sticky_weighted_strategy_uses_configured_weights_for_new_pool(tmp_path, monkeypatch):
    """sticky_weighted should choose the lowest usage/weight ratio for task starts."""
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    monkeypatch.setattr(
        "agent.credential_pool._seed_from_singletons",
        lambda provider, entries: (False, set()),
